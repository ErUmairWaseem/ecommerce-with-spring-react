...
function HomeSectionCarousel4({ data }) {
  const [activeIndex, setActiveIndex] = useState(0);
  const slidePrev = () => setActiveIndex(activeIndex - 2);
  const slideNext = () => setActiveIndex(activeIndex + 2);

  const getBox = () => {
    return document.querySelector(".carousel4-container");
  };
  const getBox2 = () => {
    return document.querySelector(".carousel4-container-list");
  };
  ...
  return (
    <div className="carousel4 bg-gray-50">
      <div className="carousel4-content relative carousel-content flex flex-col px-8 border border-blue-200 rounded-md ">
        <div className="w-full flex">
          <h2 className="text-xl font-semibold text-blue-600 cursor-pointer py-2 mx-2">
            Women's Gouns
          </h2>
        </div>
        <div className="carousel4-container pb-4 gap-2 flex scroll-smooth overflow-hidden">
          {data.map((product) => (
            <div className="carousel4-container-list p-0.5 hover:p-0 border border-gray-300 hover:border-blue-400 mx-1 flex flex-col items-center rounded-md shadow-md hover:opacity-80 cursor-pointer">
              <img
...
export default HomeSectionCarousel4;
